...
# If the value is omitted, return sends back a None. Technically, a function
# without a return statement also returns the None object automatically.

# The Python def is a true executable statement: when it runs, it creates
# a new function object and assigns it to a name.
...
# One way to understand this code is to realize that the def is much like
# an = statement: it simply assigns a name at runtime.

# IMPORTANT: defs are not evaluated until they are reached and run, and
# the code inside defs is not evaluated until the functions are later called.

# Because function definition happens at runtime, there's nothing special 
# about the function name. 
othername = func
othername()                                                   # f1

# Besides calls, functions allow arbitrary attributes to be attached to record
...
print('-' * 10 + "A.2. Scope and LEGB Rule" + '-' * 10)

# The place where you assign a name in your source code determines
# the namespace it will live in, and hence its scope of visibility.
...
# If you need to ASSIGN a name that lives at the top level of the
# module enclosing the function, you can do so by declaring it in
# a global statement inside the function.

# Names not assigned a value in the function definition are assumed
# to be enclosing scope locals. Also note that any type of assignment
# within a function classifies a name as local.

# Conversely, in-place changes to objects do not classify names as
# locals; only actual name assignments do. For instance, if the name
# L is assigned to a list at the top level of a module, a statement
# L = X within a function will classify L as a local, but L.append(X)
# will not. In the latter case, we are changing the list object that
# L references, not L itself.
...
# In other words, all names assigned inside a function def statement 
# (or a lambda, an expression we'll meet later) are locals by default.
# Functions can freely use names assigned in syntactically enclosing
# functions and the global scope, but they must declare such nonlocals
# and globals in order to CHANGE them.

# for loop statements never localize their variables to the statement
# block in any Python. List comprehensions usually localize (p.490),
...
def func4(y):
    global x
    z = x + y                                                 # local scope
    x = 999                                                   # change global variable
    p = 777                                                   # create local variable
    return z

rv = func4(1)
print(rv, x, p)                                               # (100, 999, 55)

# Because the names True and False in 2.X are just variables in the
# built-in scope and are not reserved, it's possible to reassign them
# with a statement like True = False. This statement merely redefines
# the word True for the single scope in which it appears to return False.

True = False                                                  # Redefine the word True
if True == False:
...
def glob2():
    var = 0                                                   # Change local var 
    import functions_basic                                    # Import THIS module
...
test()                                                        # also print(100) comes from somewhere (when the module import itself)
